...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while(!frontier.isEmpty()) { //checking if the frontier is empty
			SearchNode leaf = frontier.removeFirst(); // choose a leaf node and remove it from the frontier
			if(p.isGoalState(leaf.getState())) { //if node contains a goal state -> return solution
				path = leaf.getPathFromRoot(); // get the path to return
				return path;
			}else{
			explored.add(leaf); // adding the node to the explored set
			}
			for (GridPos childPos : p.getReachableStatesFrom(leaf.getState())) { //iterate through childstates
				SearchNode child = new SearchNode(childPos, leaf); 
				if( !frontier.contains(child) && !explored.contains(child)) { // only if not in the frontier or the explored set
					if (insertFront == true) { //deciding wether BFS or DFS should be used
